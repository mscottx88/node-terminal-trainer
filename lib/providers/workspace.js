...
function Workspace(
  ...
  Object.defineProperty(this, 'trainingPackageProgress', {
    get: () => {
      const rows = [['Title', 'Completion']];
      ...
      return rows.concat(
        _.map(this.trainingPackages,
          ({ exercises, title }) => [title, `${(exercises.filter(({ completed }) => completed).length / exercises.length) * 100}%`]
        )
      );
    }
  });
...
Workspace.prototype.setPreference = function (
  {
    name,
    value
  } = {}
...
Workspace.prototype.getPreference = function (
  {
    name,
    defaultValue
  } = {}
