  ...
  addScreenElement(
    ...
    if (focusable) {
      ...
      if (type === 'Textbox') {
        ...
        definition.inputOnFocus = true;
        definition.keys = true;
      }
    }
    ...
    definition.tags = true;

    if (type === 'ColumnList') {
      this[name] = new ColumnList(definition);
    } else if (type === 'DirectoryTree') {
      this[name] = new DirectoryTree(definition);
    } else {
    ...
    if (!this.tabbableElements.has(element)) {
      element.on('keypress', (ch, key) => {
        if (key && key.full === 'tab') {
          ...
          this.boundTabHandler.call(this, { previous: element });
        }
      });
    }
